...
class AnalogView extends WatchUi.WatchFace
{
	...
	// Custom tiny bitmap font for labels
	var tinyBitmapFont = null;
	var tinyBitmapFontLg = null;
    ...
    // Initialize variables for this view
        ...
        // Load tiny bitmap font for outside-LCD labels
        try { tinyBitmapFont = WatchUi.loadResource(Rez.Fonts.id_font_tiny); } catch (e) { tinyBitmapFont = null; }
        try { tinyBitmapFontLg = WatchUi.loadResource(Rez.Fonts.id_font_tiny_lg); } catch (e) { tinyBitmapFontLg = null; }
...
/* SECOND TIME ZONE */
        ...
        // LCD colours — inverted Casio A168WEGG style (dark bg, white text)
        var lcdBg;
        var lcdColour;
        var lcdDimColour;
        if (isAwake) {
            lcdBg = 0x333333;
            lcdColour = 0xFFFFFF;
            lcdDimColour = 0xAAAAAA;
        } else {
            lcdBg = 0x000000;
            lcdColour = 0x999999;
            lcdDimColour = 0x555555;
        }
        ...
        targetDc.setColor(isAwake ? 0x666666 : 0x333333, Graphics.COLOR_TRANSPARENT);
        ...
        // ===== OUTSIDE LCD =====
        drawOutsideLcd(targetDc, width, height, lcdX, lcdY, lcdW, lcdH, labelFontH);
    ...
    private function drawOutsideLcd(targetDc, width, height, lcdX, lcdY, lcdW, lcdH, labelFontH) {
        var cx = width / 2;
        var cy = height / 2;
        // -- TOP SECTION: NOMIS + ALARM CHRONO, then ELECTRO LUMINESCENCE --
        var topGap = lcdY;
        // Use custom bitmap fonts only on screens >= 390px wide; fall back to built-in on smaller displays
        var useCustomFont = (width >= 390 && tinyBitmapFont != null);
        var fallback = (Graphics has :FONT_GLANCE) ? Graphics.FONT_GLANCE : Graphics.FONT_XTINY;
        var tinyFont = useCustomFont ? tinyBitmapFont : fallback;
        var tinyFontLg = (useCustomFont && tinyBitmapFontLg != null) ? tinyBitmapFontLg : tinyFont;
        var tinyFontH = Graphics.getFontHeight(tinyFont);
        var tinyFontLgH = Graphics.getFontHeight(tinyFontLg);
        // Row 1: NOMIS (larger font, left) + ALARM CHRONO (small font, right)
        var topRow1Y = (topGap * 0.60).toNumber();
        var topTextW = targetDc.getTextWidthInPixels(topText, tinyFontLg);
        var alarmChronoW = targetDc.getTextWidthInPixels("ALARM CHRONO", tinyFont);
        var topTotalW = topTextW + 6 + alarmChronoW;
        var topStartX = cx - topTotalW / 2;
        targetDc.setColor(0xBBBBBB, Graphics.COLOR_TRANSPARENT);
        targetDc.drawText(topStartX + topTextW / 2, topRow1Y, tinyFontLg, topText, Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        targetDc.drawText(topStartX + topTextW + 6 + alarmChronoW / 2, topRow1Y, tinyFont, "ALARM CHRONO", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        // Row 2: ELECTRO LUMINESCENCE — E and L in larger font, rest in small
        // Use top-justified drawing with a common bottom line for exact bottom-alignment
        var row2Bottom = topRow1Y + tinyFontLgH / 2 + 6 + tinyFontLgH;
        var row2TopSm = row2Bottom - tinyFontH;
        var row2TopLg = row2Bottom - tinyFontLgH;
        var eW = targetDc.getTextWidthInPixels("E", tinyFontLg);
        var lectroW = targetDc.getTextWidthInPixels("LECTRO ", tinyFont);
        var lW = targetDc.getTextWidthInPixels("L", tinyFontLg);
        var uminescenceW = targetDc.getTextWidthInPixels("UMINESCENCE", tinyFont);
        var elTotalW = eW + lectroW + lW + uminescenceW;
        var elX = cx - elTotalW / 2;
        targetDc.setColor(0xBBBBBB, Graphics.COLOR_TRANSPARENT);
        targetDc.drawText(elX, row2TopLg, tinyFontLg, "E", Graphics.TEXT_JUSTIFY_LEFT);
        targetDc.drawText(elX + eW, row2TopSm, tinyFont, "LECTRO ", Graphics.TEXT_JUSTIFY_LEFT);
        targetDc.drawText(elX + eW + lectroW, row2TopLg, tinyFontLg, "L", Graphics.TEXT_JUSTIFY_LEFT);
        targetDc.drawText(elX + eW + lectroW + lW, row2TopSm, tinyFont, "UMINESCENCE", Graphics.TEXT_JUSTIFY_LEFT);

        // -- BOTTOM SECTION: ILLUMINATOR, then WATER RESIST --
        var botGap = height - lcdY - lcdH;
        var illuminatorY = lcdY + lcdH + (botGap * 0.32).toNumber();
        var waterResistY = illuminatorY + tinyFontH + 14;
        targetDc.setColor(0xBBBBBB, Graphics.COLOR_TRANSPARENT);
        targetDc.drawText(cx, illuminatorY, tinyFont, "ILLUMINATOR", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);
        // White line between ILLUMINATOR and WATER RESIST
        var illumLineY = illuminatorY + tinyFontH / 2 + 6;
        var ilW = (lcdW * 0.7).toNumber();
        targetDc.setColor(Graphics.COLOR_WHITE, Graphics.COLOR_TRANSPARENT);
        targetDc.drawLine(cx - ilW / 2, illumLineY, cx + ilW / 2, illumLineY);
        targetDc.setColor(0xBBBBBB, Graphics.COLOR_TRANSPARENT);
        targetDc.drawText(cx, waterResistY, tinyFont, "WATER RESIST", Graphics.TEXT_JUSTIFY_CENTER | Graphics.TEXT_JUSTIFY_VCENTER);

        // ===== INNER BEZEL — arcs left/right, straight lines top/bottom =====
        var bezelTop = (lcdY * 0.30).toNumber();
        var bezelBot = height - (lcdY * 0.30).toNumber();
